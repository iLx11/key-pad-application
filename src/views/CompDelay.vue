...
const keyCommit = () => {
  ...
  console.info(delayCompList.length)
  let tempObj = {
    listId: delayCompList.length == 0 ? 1 : delayCompList[delayCompList.length - 1].listId + 1,
    listStr: userKeyStr
  }
  if(curSelected.value != 0) 
    delayCompList.splice(curSelected.value, 0, tempObj)
  else 
    delayCompList.push(tempObj)
}
...
watch(
  ...
  () => {
    ...
    if (/[\u4e00-\u9fa5|A-Za-z]/.test(delayTime.delayS)) {
      configStore.notice('请输入纯数字')
      delayTime.delayS = ''
    }
    if (/[\u4e00-\u9fa5|A-Za-z]/.test(delayTime.delayMs)) {
      configStore.notice('请输入纯数字')
      delayTime.delayMs = ''
    }
  },
